...
const AGENT_INSTRUCTIONS: &str = "Zdev stores durable plans and tasks in `.zd`; the coding harness shapes, implements, and independently verifies the work. Use the harness's zdev integration for work tracked there: `zd status` and `zd next` orient and select work, `zd check` validates state, and `zd commit` records verified staged changes. Do not edit generated task indexes. If the integration is unavailable or conflicts with repository setup, ask the user how to configure it.";
const CLEANUP_SQUASH_COMMIT_MESSAGE: &str = "chore: remove zdev development record";
const GENERATED_MARKER: &str = "<!-- Generated by zd. Edit task files, not this summary. -->";
...
#[command(
    ...
    long_about = "Keep development plans and tasks in plain files under .zd.\n\nZdev creates and checks those files, selects ready tasks, checks area branches, installs coding-harness integrations, and adds stable IDs to Git commits.",
    after_help = "Start a repository:\n  1. Check your integration:       zd skill check <HARNESS> --scope user\n  2. Choose personal, project, or pull-request record storage:\n     zd init --record <POLICY>\n  3. Create or switch branches, then:\n     zd area create <TAG> --title <TITLE> --objective <OBJECTIVE>\n\nIf the integration check reports missing or conflict, run `zd skill install <HARNESS>`. Run `zd <COMMAND> --help` for command-specific details."
)]
pub struct Cli {
...
enum Command {
    ...
    ///
    /// Prints an ID without changing Git state. `zd commit` generates and adds
    /// an ID automatically.
    ChangeId,
...
enum AreaCommand {
    ...
    /// .zd/<TAG>. When --branch is omitted, the checked-out branch owns the area.
    Create {
...
enum ChangeCommand {
    ...
    Lookup {
        /// Stable ID beginning with Z, as printed by `zd change inspect`
        change_id: String,
    },
}
...
fn resolve_root(explicit: Option<&Path>) -> Result<PathBuf, ZdevError> {
    ...
    if let Some(root) = current
        .ancestors()
        .find(|path| path.join(".zd/config.toml").is_file())
    {
...
fn initialize(root: &Path, record: Option<RecordPolicy>) -> Result<CommandOutput, ZdevError> {
    let zd = root.join(".zd");
    if zd.exists() {
    ...
    fs::create_dir(&zd)
        .map_err(|error| ZdevError::io(format!("Cannot create {}", zd.display()), error))?;
    let name = root
        .file_name()
    ...
    let trunk = match config.project.trunk.as_deref() {
        ...
        None => "HEAD is detached, so no project trunk was recorded. Set one with:\n  zd config trunk <branch>".to_owned(),
    };
    let record_text = match record {
        Some(RecordPolicy::PullRequest) => {
            "\nRecord policy: pull-request. .zd is tracked for pull-request review and must be cleaned with `zd cleanup squash` before squash merge.\n"
        }
        Some(policy) => match policy {
            RecordPolicy::Personal => "\nRecord policy: personal. Keep .zd local to this clone.\n",
            RecordPolicy::Project => {
                "\nRecord policy: project. Track .zd as shared project state.\n"
            }
            RecordPolicy::PullRequest => unreachable!(),
        },
        None => "",
    };
    let record_json = record.map(|policy| {
        if policy == RecordPolicy::PullRequest {
            json!({
                "policy": policy.as_str(),
                "tracked_for": "pull-request-review",
                "cleanup_required_before": "squash-merge",
                "cleanup_command": ["cleanup", "squash"],
                "notice": ".zd is tracked for pull-request review and must be cleaned before squash merge"
            })
        } else {
...
fn cleanup_squash(root: &Path) -> Result<CommandOutput, ZdevError> {
    ...
    match config.project.record {
        ...
        None => {
            return Err(ZdevError::new(
                "Cannot clean the zdev record for squash: .zd/config.toml has no record policy; initialize with `zd init --record pull-request`",
            ));
        }
    }
    ...
    let tracked = git_output_bytes(root, &["ls-files", "-z", "--", ".zd"])?
        .split(|byte| *byte == 0)
        .filter(|path| !path.is_empty())
        .map(|path| {
            String::from_utf8(path.to_vec()).map_err(|_| {
                ZdevError::new("Cannot clean a tracked .zd path whose name is not valid UTF-8")
            })
        })
        .collect::<Result<Vec<_>, _>>()?;
    if tracked.is_empty() {
        return Err(ZdevError::new(
            "Cannot clean the zdev record because no tracked .zd files exist on this branch",
        ));
    }
    ...
    let removed = remove
        ...
        .output()
        .map_err(|error| ZdevError::io("Cannot run git rm for tracked .zd files", error))?;
    if !removed.status.success() {
        return Err(ZdevError::new(format!(
            "git rm for tracked .zd files failed before the cleanup commit: {}",
    ...
    let committed = ProcessCommand::new("git")
        ...
        .map_err(|error| {
            ZdevError::io(
                "Cannot run the cleanup commit; the tracked .zd deletion remains staged for inspection and recovery",
                error,
            )
        })?;
    if !committed.status.success() {
        return Err(ZdevError::new(format!(
            "Could not commit the cleanup. The tracked .zd deletion remains staged for inspection and recovery: {}",
    ...
    Ok(CommandOutput::new(
        format!(
            "Removed {} tracked .zd files and committed {revision} on {branch}: {CLEANUP_SQUASH_COMMIT_MESSAGE}",
            tracked.len()
        ),
        json!({
            "schema_version": SCHEMA_VERSION,
...
fn read_config(root: &Path) -> Result<Config, ZdevError> {
    let path = root.join(".zd/config.toml");
...
fn list_areas(root: &Path) -> Result<Vec<AreaMetadata>, ZdevError> {
    ...
    for entry in fs::read_dir(root.join(".zd"))
...
fn effective_base(root: &Path, config: &Config, area: &AreaMetadata) -> Result<String, ZdevError> {
    configured_effective_base(root, config, area)?.ok_or_else(|| {
        ZdevError::new("Project trunk is not configured; set it with `zd config trunk <branch>`")
    })
}
...
fn require_base_anchor(area: &AreaMetadata) -> Result<&str, ZdevError> {
    area.base_commit.as_deref().ok_or_else(|| {
        ZdevError::new(format!(
            "Area {} has no recorded base commit; rebind its branch with `zd area bind {} <branch>`",
            area.tag, area.tag
        ))
    })
}
...
fn require_fresh_area_link(root: &Path, tag: &str) -> Result<(), ZdevError> {
    ...
    if base_is_ancestor(root, &base, &branch) == Some(true) {
        ...
        if anchor != base_tip {
            return Err(ZdevError::new(format!(
                "Area {tag} needs its current base recorded. Run `zd area rebase {tag}`"
            )));
        }
        return Ok(());
    }
    ...
    {
        return Err(ZdevError::new(format!(
            "Cannot verify area {tag}'s base on branch {branch}. Rebind the area with `zd area bind {tag} <branch>`"
        )));
    }
    require_linear_child_history(root, anchor, &branch)?;
    Err(ZdevError::new(format!(
        "Area {tag} is stale. Run `zd area rebase {tag}` to rebase {branch} onto {base}"
    )))
}
...
fn managed_rebase_guidance(tag: &str) -> String {
    format!(
        "Resolve and stage the conflicts, then run `zd area rebase {tag} --continue`. To cancel, run `zd area rebase {tag} --abort`. The equivalent Git commands are `git rebase --continue` and `git rebase --abort`"
    )
}
...
fn continue_managed_rebase(root: &Path, tag: &str) -> Result<CommandOutput, ZdevError> {
    ...
    {
        return Err(ZdevError::new(format!(
            "Cannot continue: Git's rebase state no longer matches area {tag}. Run `zd area rebase {tag} --abort`, then start the rebase again"
        )));
    }
    let output = ProcessCommand::new("git")
...
fn rebase_area(
    ...
    {
        return Err(ZdevError::new(format!(
            "Cannot rebase area {tag}: its recorded base is unavailable on {branch}. Rebind it with `zd area bind {tag} <branch>`"
        )));
    }
    require_linear_child_history(root, &anchor, &branch)?;
    let output = ProcessCommand::new("git")
...
fn select_area(root: &Path, requested: Option<&str>) -> Result<String, ZdevError> {
    ...
    match active.as_slice() {
        ...
        _ => Err(ZdevError::new(format!(
            "Specify an area with `zd next <area>`. Open areas: {}",
            active.join(", ")
        ))),
    }
}
...
fn publish_task_and_index(
    ...
    let index_path = root.join(".zd").join(area).join("TASKS.md");
...
fn check_output(root: &Path, requested: Option<&str>) -> Result<CommandOutput, ZdevError> {
    ...
    for area in areas {
        validate_brief(&root.join(".zd").join(&area.tag).join("brief.md"))?;
        let tasks = load_tasks(root, &area.tag)?;
        let expected = render_index(&area.tag, &tasks)?;
        let path = root.join(".zd").join(&area.tag).join("TASKS.md");
...
fn read_recorded_guidance(root: &Path) -> Result<Option<String>, ZdevError> {
    if !root.join(".zd/config.toml").is_file() {
        return Ok(None);
    }
...
fn record_guidance(root: &Path, source: &str) -> Result<(), ZdevError> {
    ...
    fs::create_dir_all(root.join(".zd"))
        .map_err(|error| ZdevError::io("Cannot create .zd", error))?;
    write_config(root, &config)
}
...
fn inspect_guidance(
    ...
    let fallback = root.join(".zd/guidance.md");
...
fn commit(root: &Path, message: &str, body: &[String]) -> Result<CommandOutput, ZdevError> {
    ...
    if body.iter().any(|paragraph| {
        ...
        return Err(ZdevError::new(format!(
            "Commit body must not contain a {CHANGE_ID_TRAILER} trailer; zd adds one automatically"
        )));
    }
    let _lock = ZdevStateLock::acquire(root)?;
    let change_id = generate_change_id()?;
