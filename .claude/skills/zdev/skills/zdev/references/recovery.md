...
Use `zd area rebase <area>` for ordinary trunk updates and parent-area updates.
...
Run `zd status <area> --format json` again before task completion. Completion
...
or finalize a rebase completed directly through Git. Then rerun `zd next`.
