...
- [ ] An unchanged checkout reaches blocker classification, and new investigation or validation evidence can lead to a concrete continuation within the task.
- [ ] Repeated attempts stop when the same obstacle remains unresolved with no actionable next step; existing safety and external-prerequisite stops still work.
- [ ] Terminal verifier-blocker output includes its findings, including a concrete missing prerequisite or user decision, through existing public report fields.
- [ ] Focused executable tests cover unchanged-state continuation, a repeated genuine impasse, and preservation of verifier findings; regenerated Claude integration matches its source.
