...
- [ ] Import JSON includes ready task IDs in stable numeric order after the published graph is validated.
- [ ] The result is correct when existing tasks remain ready and newly imported tasks are blocked.
- [ ] Task-creation guidance uses the returned frontier and no longer calls tasks list after successful import.
